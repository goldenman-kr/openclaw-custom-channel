...
- job 상태 SQLite 저장: 부합
- UI/sidebar/drawer 구현: 부합
- token-level streaming 및 SSE 기반 partial persistence: 현재 코드 기준 구현됨
...
- Web/PWA 클라이언트-서버 간 WebSocket 실시간 sync: 현재 코드는 SSE/polling 및 Gateway 내부 WebSocket subscriber 중심이며, 브라우저 실시간 sync 채널은 별도 과제로 남긴다.
...
## 2026-05-02 sync message auth fix

모바일 PWA에서 로그인 세션으로 메시지를 보낼 때 `Authorization header is required.` 오류가 보였다.

조치:
- `/v1/message`의 `x-openclaw-sync: 1` 경로도 cookie auth context를 먼저 사용하도록 수정했다.
- sync 경로에서도 일반 메시지 경로와 동일하게 conversation owner visibility와 runtime workspace scope를 적용한다.
- API key fallback은 유지한다.
- PWA service worker cache를 `openclaw-web-channel-v141`로 올려 클라이언트 갱신을 유도했다.

## 2026-05-02 queued message cookie auth fix

모바일 PWA 일반 전송에서 여전히 `Authorization header is required.`가 발생했다.

원인:
- HTTP route 진입 시에는 cookie auth로 검증됐지만, queued job 실행 단계에서 `MessageJobRunner`가 `handlePostMessage()`를 다시 호출할 때 auth context를 넘기지 않아 legacy API key 검사를 다시 탔다.

조치:
- `MessageJob`에 `authContext`를 추가했다.
- `/v1/message` async route에서 검증된 cookie/API key auth context를 job에 보관한다.
- `MessageJobRunner`가 job 실행 시 `authContext`를 `handlePostMessage()`로 전달한다.
- cookie auth context만으로 Authorization header 없이 `handlePostMessage()`가 동작하는 테스트를 추가했다.
- 추가로 job authContext/runtimeWorkspace는 서버 내부 실행 정보이므로 `/v1/jobs` 및 SSE 응답에는 노출하지 않도록 public job DTO로 정리했다.

## 2026-05-02 login-first home UX fix

일반 사용자 로그인 후 첫 화면에 "API Key" 설정 안내가 남아 있어 login-first UX와 맞지 않았다.

조치:
- API key 중심 안내 문구를 로그인 중심 안내로 변경했다.
- 로그인 사용자는 대화가 없을 때 초기 로딩 후 자동으로 새 대화를 생성해 바로 홈/대화 화면으로 진입하도록 했다.
- `ensureActiveConversation()`에서 더 이상 API key를 요구하지 않도록 했다.
- PWA service worker cache를 `openclaw-web-channel-v142`로 올렸다.

## 2026-05-02 login home rerender fix

로그인 성공 후에도 초기 로그인 전 홈 화면(`로그인 후 대화를 시작할 수 있습니다.`, `설정 열기`)이 남아 있었다.

원인:
- 앱 시작 시 `renderHome()`이 auth 확인 전에 먼저 실행된다.
- `loadCurrentUser()` 성공 후 initial conversation id가 없으면 홈을 다시 렌더링하지 않고 return해서 pre-auth 화면이 그대로 남았다.

조치:
- 로그인 사용자 확인 및 conversation refresh 후 initial conversation id가 없으면 `renderHome()`/`renderConversationList()`를 다시 호출한다.
- PWA service worker cache를 `openclaw-web-channel-v143`으로 올렸다.

## 2026-05-02 login submit rerender fix

로그인 버튼으로 로그인에 성공한 직후 여전히 pre-auth 홈 화면이 남았다.

원인:
- 앞선 수정은 페이지 로드 시 `loadCurrentUser()` 성공 경로만 처리했다.
- 실제 로그인 form submit 성공 경로에서는 `refreshConversations()` 후 initial conversation id가 없으면 아무 화면 전환도 하지 않았다.

조치:
- 로그인 submit 성공 후에도 대화가 없으면 새 대화를 자동 생성한다.
- initial conversation id가 없고 기존 대화가 있으면 `renderHome()`/`renderConversationList()`를 다시 호출한다.
- PWA service worker cache를 `openclaw-web-channel-v144`로 올렸다.

## 2026-05-02 per-user identity context draft

새 일반 사용자(`soprano`)로 대화해도 OpenClaw가 상대를 Eddy로 부르는 문제가 있었다.

판단:
- 데이터/대화 권한 분리는 됐지만 OpenClaw runtime은 여전히 shared `main` agent/global workspace memory의 영향을 받는다.
- `gateway-openai` transport에서는 per-request workspace cwd 강제가 확인되지 않았으므로, 사용자별 workspace identity 파일 + prompt metadata 주입으로 1차 완화한다.

조치:
- 일반 사용자 workspace scope 생성/조회 시 `<userDir>/WEBCHAT_USER.md`를 자동 생성한다.
- 파일에는 현재 webchat user id/username/display name, "Eddy가 아님", Eddy-specific memory를 적용하지 말라는 지침을 기록한다.
...
## 2026-05-02 streaming pending bubble cleanup

답변이 이미 도착했는데 Web/PWA 화면에 `응답을 처리 중입니다…` pending 버블이 남는 현상을 확인했다.
새로고침하면 사라지는 것으로 보아 서버 저장 상태가 아니라 클라이언트 DOM/streaming timer 상태 문제로 판단했다.

원인:
- token streaming 중 `scheduleStreamingIdleCheckpoint()`가 job별 10초 idle timer를 예약한다.
- job이 terminal 상태(`completed`/`failed`/`cancelled`)가 되어도 기존 timer와 `streamingTextByJob` buffer가 정리되지 않았다.
- 늦게 실행된 idle checkpoint가 최종 응답 DOM을 partial node로 분리하고 원 job node를 다시 pending placeholder로 렌더링할 수 있었다.
- DB/history에는 최종 assistant message가 정상 저장되어 있어 새로고침 시 canonical history render로 복구됐다.

반영된 조치:
- `server/public/app.js`에 `clearStreamingState(jobId)`를 추가했다.
- 이 함수는 해당 job의 `streamingIdleTimers` timeout을 clear/delete하고 `streamingTextByJob` buffer를 삭제한다.
- SSE 경로에서 `expired` 이벤트를 받거나 terminal job event를 받으면 `clearStreamingState(jobId)`를 먼저 호출한 뒤 pending job storage를 정리한다.
- polling fallback 경로에서도 terminal job, history상 resolved job, 404 expired job 판단 시 `clearStreamingState(jobId)`를 호출한다.

현재 코드상 주의:
- 현재 반영은 streaming timer/buffer 정리 중심이다.
- DOM node의 `_streamingText` 속성을 직접 삭제하거나 terminal 직후 history refresh를 강제하는 별도 force 옵션은 아직 없다.
- 다만 timer가 제거되면 문제의 직접 원인인 “완료 후 늦은 checkpoint 재실행”은 차단된다.

검증:
- `node --check server/public/app.js` 통과
- `git diff --check` 통과

## 2026-05-02 streaming pending bubble cleanup recheck

외부에서 패치가 완료됐다는 알림 후 현재 working tree 기준으로 Web/PWA 클라이언트 구현을 다시 확인했다.

확인한 현재 구현:
- `server/public/app.js`의 `clearStreamingState(jobId)`는 다음 상태만 정리한다.
  - `streamingIdleTimers`의 해당 job timeout clear/delete
  - `streamingTextByJob`의 해당 job buffer delete
- 호출 지점은 다음 terminal/종료 경로에 들어가 있다.
  - SSE `expired` 이벤트
  - SSE terminal `job` 이벤트(`completed`/`failed`/`cancelled`)
  - polling fallback에서 terminal job 상태 확인
  - polling 중 history에서 job이 resolved된 것으로 확인된 경우
  - polling 중 job API가 404를 반환해 expired로 보는 경우

현재 코드상 확인된 경계:
- `scheduleStreamingIdleCheckpoint()` 내부에서는 여전히 idle 시 partial node(`job_id:partial`)를 만들고 원 job node를 pending placeholder로 되돌릴 수 있다.
- 하지만 terminal/expired/resolved 경로에서 timer가 clear되므로, 정상 완료 이후 늦은 idle checkpoint가 새 pending 버블을 되살리는 직접 경로는 차단된다.
- 현재 구현은 DOM node의 `_streamingText` 속성 삭제나 terminal 직후 force history rerender까지는 하지 않는다. 필요 시 후속 hardening으로 추가할 수 있다.
- 이번 재확인 시점의 `server/public/app.js` diff는 streaming cleanup 14줄 추가 수준이며, force refresh 계열 변경은 보이지 않는다.

검증:
- `node --check server/public/app.js` 통과
- `git diff --check` 통과

## 2026-05-03 GitHub 반영 기준 정리 및 PLAN 재검토

기준 커밋:
- `e8c93a1 Stabilize PWA streaming and native controls`
- `fb262c3 Wait for Gateway agent event subscription readiness`

이번 GitHub 반영분에서 확인한 구현:

### 1) Web/PWA native command 보강

변경 파일:
- `server/src/http/nativeCommands.ts`
- `server/src/http/messageRoutes.ts`
- `server/src/http/nativeCommands.test.ts`

적용 내용:
- `/status`는 가능하면 OpenClaw runtime의 direct status formatter를 우선 사용하고, 실패 시 web session status card/fallback status로 내려오게 했다.
- provider usage 표시(`📊 Usage`)는 dist runtime을 동적 import하되, 응답 경로를 막지 않도록 timeout + cache + background refresh 방식으로 바꿨다.
- `/model`은 admin만 변경 가능하도록 제한했고, 응답 문구에 전역 override임을 명시했다.
- 일반 사용자는 현재 모델 확인만 가능하다.
- 관련 회귀 테스트(`nativeCommands.test.ts`)를 추가했다.

의미:
- `/status`가 느려서 PWA가 멈춘 것처럼 보이던 리스크를 줄였다.
- emergency model override가 일반 사용자에게 노출되는 위험을 줄였다.

### 2) queued message streaming 안정화 및 partial persistence 개선
...
- `server/src/events/SseJobEventPublisher.ts`
- `server/src/openclaw/GatewayOpenAiOpenClawClient.ts`
- `server/src/openclaw/GatewayAgentEventSubscriber.ts`
- `server/src/index.ts`

적용 내용:
- `ChatRuntimeCallbacks`에 `onAgentEvent()`를 추가했다.
- `SseJobEventPublisher`가 token뿐 아니라 `agent` 이벤트도 SSE로 publish할 수 있게 했다.
- `GatewayOpenAiOpenClawClient`는 sessionKey 기준으로 Gateway websocket event stream을 subscribe하는 `GatewayAgentEventSubscriber`를 사용한다.
- subscriber는 websocket open만 기다리지 않고 `connect.challenge -> connect -> sessions.subscribe`까지 끝나야 ready로 간주하도록 보강했다.
- `MessageJobRunner`는 token stream을 단일 버퍼로만 들고 있지 않고, idle/tool boundary 기준으로 partial assistant message(`job_id:partial`)를 DB에 checkpoint 저장한다.
- tool start agent event가 오면 현재 stream segment를 partial message로 경계 저장하도록 했다.
- 최종 assistant reply 저장 시 이미 persisted partial text와 중복되는 앞부분은 잘라 canonical final message를 저장한다.
- server startup 시 오래된 `queued/running` jobs를 `cancelled`로 정리하는 stale job cleanup을 추가했다.

의미:
- 장시간 streaming/tool 호출이 섞인 답변에서도 partial text 보존과 pending placeholder 관리가 더 안정적이다.
- PWA 재접속/히스토리 새로고침 시 partial checkpoint와 final message 정합성이 좋아졌다.
- startup 후 stale queued/running job이 계속 남아 pending UX를 오염시키는 문제를 줄였다.

### 3) PWA pending bubble cleanup 재확인

변경 파일:
- `server/public/app.js`

적용 내용:
- `clearStreamingState(jobId)`가 SSE/polling terminal 경로에 연결되어, 완료 후 늦은 idle checkpoint가 pending bubble을 되살리는 경로를 차단한다.
- 현재 구현은 timer/buffer cleanup 중심이며, `_streamingText` DOM 속성 삭제나 force rerender는 포함하지 않는다.

### 4) 검증

GitHub 반영 전/후 확인한 검증:
- `npm run typecheck` 통과
- `npm test` 통과 (33/33)
- `npm run build` 통과
- `node --check server/public/app.js` 통과
...
`MULTI_SESSION_PLAN.md`의 원래 핵심 범위(다중 conversation 저장/전환, active conversation 연결, 데스크톱 sidebar, 모바일 drawer, conversation 기반 history/message/job, SSE 방향 검토)는 현재 구현/기록 기준으로 실질적으로 충족된 상태로 판단한다.

따라서 **원래 multi-session PLAN의 본 작업 범위 기준으로는 별도의 필수 잔여 작업은 없다.**

다만 아래 항목들은 여전히 “후속 확장/개선 과제”로 남는다.

1. **고급 취소/중단 semantics 정교화**
   - 현재도 cancel 경로와 abort 연동은 있으나 PLAN 7.5에 적힌 수준의 취소 요청됨/중단됨 UX 구분, transport별 강제 종료 정책은 더 다듬을 여지가 있다.

2. **WebSocket 실시간 sync 재검토**
   - 현재 SSE/polling 구조와 Gateway 내부 WebSocket subscriber는 있으나, PLAN 7.3/7.4의 브라우저 실시간 sync 채널은 별도 과제로 남는다.

3. **멀티유저 hardening/운영정책 정교화**
   - auth/login/logout/me, `users`/`auth_sessions`, `owner_id`, media/workspace guard, user-admin script 등 핵심 구조는 코드에 존재한다.
   - 다만 cross-user admin surface, audit, quota, 명시적 권한모델 hardening은 별도 운영 과제로 남는다.

4. **기타 장기 검토 메모**
   - 인증 해시 알고리즘 선택 재검토(`argon2id` 등)
   - 민감 audit log 정책
   - workspace/symlink/read guard의 추가 hardening

정리하면:
- **현재 PLAN의 본선 목표는 완료로 봐도 무방**하다.
- 남은 것은 대부분 새 요구가 생길 때 따로 꺼낼 “확장 과제”다.

## 2026-05-03 GitHub 반영 기준 재확인 메모

확인 시점 상태:
- 현재 작업 브랜치: `feature/pwa-agent-event-split`
- `HEAD`는 `origin/feature/pwa-agent-event-split`와 동일했다.
- 확인된 최신 GitHub 반영 커밋은 `fb262c3 Wait for Gateway agent event subscription readiness`였다.

주의:
- 확인 시점 working tree에는 아래 로컬 수정이 추가로 있었지만, 이는 **GitHub 반영 기준에는 포함하지 않았다.**
  - `MULTI_SESSION_IMPL.md`
  - `server/public/app.js`
  - `server/public/index.html`
  - `server/public/sw.js`
  - `server/src/index.ts`
  - `server/src/runtime/MessageJobRunner.ts`
- 따라서 이 문서의 PLAN 재검토 결론은 **GitHub에 이미 올라간 구현만 기준**으로 판단했다.

재검토 결론:
- `MULTI_SESSION_PLAN.md`의 원래 본선 범위(멀티 conversation 저장/조회/전환, conversation 기반 history/message/job 연결, Web/PWA sidebar/drawer, streaming/SSE 안정화 방향)는 GitHub 반영 기준으로 충족된 상태다.
- **필수로 남아 있는 본선 작업은 없다고 판단한다.**
- 남아 있는 것은 취소 semantics 정교화, 멀티유저 hardening/운영정책 정교화, WebSocket 실시간 sync 재검토 같은 후속 과제들이다.
